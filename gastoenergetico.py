...
connection = client.connect("http://187.188.90.137:4200/", username="crate",timeout=5)
...
while i<=largo:
  ...
  if len(indiceini)==3:
    data=pd.DataFrame(asiganacion(df.iloc[indiceini[0]].tolist(),df.iloc[indiceini[1]].tolist(),df.iloc[indiceini[2]].tolist())).T 
    dfaux=dfaux.append(data, ignore_index=True)  
    i=i+3
    continue
  if len(indicefin)==3:
    data=pd.DataFrame(asiganacion(df.iloc[indicefin[0]].tolist(),df.iloc[indicefin[1]].tolist(),df.iloc[indicefin[2]].tolist())).T 
    dfaux=dfaux.append(data, ignore_index=True)
    i=i+3
    continue
  if len(indiceini)==2 :
    data=pd.DataFrame(asiganacion(df.iloc[indiceini[0]].tolist(),df.iloc[indiceini[1]].tolist(),"")).T 
    dfaux=dfaux.append(data, ignore_index=True)
    i=i+2
    continue
  if len(indicefin)==2:
    data=pd.DataFrame(asiganacion(df.iloc[indicefin[0]].tolist(),df.iloc[indicefin[1]].tolist(),"")).T 
    dfaux=dfaux.append(data, ignore_index=True)
    i=i+2
    continue
...
conjunto=[time,cinturaejesx,cinturaejesy,cinturaejesz,manoejesx,manoejesy,manoejesz,piernaejesx,piernaejesy,piernaejesz]
s = pd.DataFrame(conjunto).T
s.columns =['time','cinturaejesx',	'cinturaejesy',	'cinturaejesz',	'piernaejesx',	'piernaejesy',	'piernaejesz',	'manoejesx',	'manoejesy',	'manoejesz']
...
epoca=10
if cinturaejesx.size > 0:
  cinturaejesx_counts = convert_counts(cinturaejesx, time, epoch=epoca, rectify='full', integrate='trapezoid', plot=False)
  cinturaejesy_counts = convert_counts(cinturaejesy, time, epoch=epoca, rectify='full', integrate='trapezoid', plot=False)
  cinturaejesz_counts = convert_counts(cinturaejesz, time, epoch=epoca, rectify='full', integrate='trapezoid', plot=False)

if manoejesx.size > 0:
  manoejesx_counts = convert_counts(manoejesx, time, epoch=epoca, rectify='full', integrate='trapezoid', plot=False)
  manoejesy_counts = convert_counts(manoejesy, time, epoch=epoca, rectify='full', integrate='trapezoid', plot=False)
  manoejesz_counts = convert_counts(manoejesz, time, epoch=epoca, rectify='full', integrate='trapezoid', plot=False)

if piernaejesx.size > 0:
  piernaejesx_counts = convert_counts(piernaejesx, time, epoch=epoca, rectify='full', integrate='trapezoid', plot=False)
  piernaejesy_counts = convert_counts(piernaejesy, time, epoch=epoca, rectify='full', integrate='trapezoid', plot=False)
  piernaejesz_counts = convert_counts(piernaejesz, time, epoch=epoca, rectify='full', integrate='trapezoid', plot=False)

if cinturaejesx.size > 0:
  cinturaejesx_f2_counts = convert_counts(cinturaejesx_f2, time, epoch=epoca, rectify='full', integrate='simpson', plot=False)
  cinturaejesy_f2_counts = convert_counts(cinturaejesy_f2, time, epoch=epoca, rectify='full', integrate='simpson', plot=False)
  cinturaejesz_f2_counts = convert_counts(cinturaejesz_f2, time, epoch=epoca, rectify='full', integrate='simpson', plot=False)
if manoejesx.size > 0:
  manoejesx_f2_counts = convert_counts(manoejesx_f2, time, time_scale='ms', epoch=epoca, rectify='full', integrate='simpson', plot=False)
  manoejesy_f2_counts = convert_counts(manoejesy_f2, time, time_scale='ms', epoch=epoca, rectify='full', integrate='simpson', plot=False)
  manoejesz_f2_counts = convert_counts(manoejesz_f2, time, time_scale='ms', epoch=epoca, rectify='full', integrate='simpson', plot=False)
if piernaejesx.size > 0:
  piernaejesx_f2_counts = convert_counts(piernaejesx_f2, time, time_scale='ms', epoch=epoca, rectify='full', integrate='simpson', plot=False)
  piernaejesy_f2_counts = convert_counts(piernaejesy_f2, time, time_scale='ms', epoch=epoca, rectify='full', integrate='simpson', plot=False)
  piernaejesz_f2_counts = convert_counts(piernaejesz_f2, time, time_scale='ms', epoch=epoca, rectify='full', integrate='simpson', plot=False)

if cinturaejesx.size > 0:
  cinturavm = vector_magnitude(cinturaejesx_counts, cinturaejesy_counts, cinturaejesz_counts)
if manoejesx.size > 0:
  manovm = vector_magnitude(manoejesx_counts, manoejesy_counts, manoejesz_counts)
if piernaejesx.size > 0:
  piernavm = vector_magnitude(piernaejesx_counts, piernaejesy_counts, piernaejesz_counts)

if cinturaejesx.size > 0:
    cinturaFvm = vector_magnitude(cinturaejesx_f2_counts, cinturaejesy_f2_counts, cinturaejesz_f2_counts)
if manoejesx.size > 0:
  manoFvm = vector_magnitude(manoejesx_f2_counts, manoejesy_f2_counts, manoejesz_f2_counts)
if piernaejesx.size > 0:
  piernaFvm = vector_magnitude(piernaejesx_f2_counts, piernaejesy_f2_counts, piernaejesz_f2_counts)
